      ...
      <div className="w-full h-auto relative flex md:flex-row flex-col md:justify-between md:items-end">
            ...
            if (isFeatured)
              return (
                <CLink
                  ...
                  onClick={() => onClose()}
                  className={`font-main w-fit cursor-pointer uppercase text-[clamp(1.35rem,7vw,3rem)] md:text-5xl 2xl:text-6xl font-medium leading-none hover:opacity-60! hover:transition-opacity hover:duration-500 ${BUTTON_RADIUS_S} px-2.5 py-2 md:mt-6 sm:mt-4 mt-2`}
                  style={{
                ...
                >
                  <span>/</span>{" "}
                  <span className={"px-2.5 pr-5"}>{label}</span>
                </CLink>
              );
            else
              return (
                <motion.button
                  ...
                  className="font-main w-fit cursor-pointer uppercase text-[clamp(1.35rem,7vw,3rem)] md:text-5xl 2xl:text-6xl font-medium leading-none hover:opacity-60! hover:transition-opacity hover:duration-500"
                >
        ...
        <div className="w-full max-w-110 h-fit flex flex-col md:items-end items-start relative gap-2.5">
          ...
          <motion.q
            key={"q-key"}
            variants={itemVariants}
            className="font-second md:text-lg text-base md:mr-2 ml-0.5 opacity-75 tracking-wide"
          >
            {messages.nav.quote}
          </motion.q>
          <motion.div
